  ...
  <div>
    <div class="content-card">
      <div class="card-header">
        <span class="card-title">Todos los reportes de domicilio</span>
      </div>
      <!-- Filtros -->
      ...
      <div class="table-wrap">
         <table class="data-table">
            ...
            <tbody>
               ...
               >
                  <td class="td-folio">{{ row.Name }}</td>
                  <td>{{ row.Nombre_del_cliente }}</td>
                  <td>{{ row.RFC }}</td>
                  <td>
                     {{ row.Asesor?.name }}
                  </td>
                  <td>{{ row.Nombre_de_Master_broker?.name }}</td>
                  <td>{{ row.Registrar_datos_del_domicilio_particular }}</td>
...
const columns = [
  { key: 'Name',   label: 'Folio', filterable: true  },
  { key: 'Nombre_del_cliente',   label: 'Nombre cliente', filterable: true  },
  { key: 'RFC', label: 'RFC', filterable: true},
  { key: 'Asesor',   label: 'Asesor', filterable: true  },
  { key: 'Nombre_de_Master_broker',    label: 'Master broker', filterable: true },
  { key: 'Registrar_datos_del_domicilio_particular',    label: 'Domicilio', filterable: true },
   { key: 'Created_Time',      label: 'Fecha creación', filterable: false }

]

const estadoOpts = ['Activo', 'Pendiente', 'Cancelado', 'Completado']
...
async function fetchData(page = 1) {
   ...
   try {
      ...
      hasNext.value = rows.value.length === perPage
   } catch (e) {
...
const filters = ref({ Name: '', Nombre_del_cliente: '', RFC: '', Asesor: '', Nombre_de_Master_broker: '', Registrar_datos_del_domicilio_particular:''})
const activeFilters = ref({ Name: '', Nombre_del_cliente: '', RFC: '', Asesor: '', Nombre_de_Master_broker: '', Registrar_datos_del_domicilio_particular:''})
...
const filteredRows = computed(() => {
  let result = rows.value.filter(row => {
    return columns.every(col => {
      ...
      if (col.key === 'Asesor') {
         return row.Asesor?.name?.toLowerCase().includes(f)
      }

      if (col.key === 'Nombre_de_Master_broker') {
         return row.Nombre_de_Master_broker?.name?.toLowerCase().includes(f)
      }
...
const abrirDetalle = (row) => {
    window.open(
      `https://ahmex.com.mx/procesos/reporte_visita_pdfv3.php?id=${row.id}`, 
      '_blank',
